package tech.codegarage.recyclebin.observer;
...
import static tech.codegarage.recyclebin.util.AllConstants.BUFFER_SIZE;
import static tech.codegarage.recyclebin.util.AllConstants.HIDDEN_DIRECTORY;
import static tech.codegarage.recyclebin.util.AllConstants.HIDDEN_FOLDER_NAME;
import static tech.codegarage.recyclebin.util.AllConstants.INTENT_FILTER_ACTIVITY_UPDATE;
import static tech.codegarage.recyclebin.util.AllConstants.KEY_INTENT_EXTRA_UPDATE;
 ...
 */
public class RecoveryObserver extends FileObserver {

    private String mPath;
    private int mMask;
    private Context mContext;
    private EventListener mListener;
    private String TAG = RecoveryObserver.class.getSimpleName();
...
//    private int counterWatcher = 0, counterUpdate = 0;
    ...
    public RecoveryObserver(Context context, String path, int mask, EventListener listener) {
        super(path, mask);
    ...
    private boolean isValidFile(File file) {
        return
                !file.getAbsolutePath().contains(".db")
                        ...
                        && !file.getAbsolutePath().contains(HIDDEN_FOLDER_NAME)
...
//                    recover(recoveryFileInfo);
                ...
                case FileObserver.DELETE_SELF:
                    copyFile(recoveryFileInfo);
//                    recover(recoveryFileInfo);
                    RecoveryObserver.this.stopWatching(filePath);
                    break;
                case CREATE:
                    if (isValidDirectory(file)) {
...
//                        myFileObserverHashMap.put(file, fileObserver);
            ...
            RecoveryObserver.this.notify(event, file);
        }
    }
